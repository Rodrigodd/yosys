...
struct TribufWorker {
	...
	void check_tribuf_signals_consistency(pool<SigBit> const &tribuf_signals)
	{
		...
		// check tribuf_signals is still consistent
			...
			if (cell->type.in(ID($mux), ID($_MUX_))) {
				...
				if (is_a_all_z) {
					...
					for (auto bit : sigmap(cell->getPort(ID::Y)))
						tribuf_signals.insert(bit);
					log_cell(cell);
					log("Replaced mux A %s by tribuf\n", log_id(cell));
			...
			while (!added_tribufs.empty()) {
				...
				for (auto it : current_tribufs) {
					...
					for (auto cell : know_muxes.at(it)) {
						...
						log("tribuf %s is driving %s, through %s\n", log_id(tribuf), log_id(cell), log_signal(it));
						...
						if (cell->type == ID($tribuf) || cell->type == ID($_TBUF_)) {
							...
							// get only the input signals that are driven by the tribufk
							...
							if (extracted_y == input) {
								...
								// cell->setParam(ID::WIDTH, GetSize(extracted_a));
								log_assert(cell->getParam(ID::WIDTH).as_int() == GetSize(extracted_a));
							} else {
								...
								log_debug("splitting %s into (%s, %s) and (%s, %s)\n", log_id(cell->name),
									  log_signal(a2), log_signal(extracted_a2), log_signal(y2),
									  log_signal(extracted_y2));
	...
	void merge(RTLIL::SigBit sig, dict<SigBit, pool<Cell *>> const &driving_cells, pool<SigBit> &tribuf_signals)
	{
		...
		if (no_tribuf) {
			...
			for (auto bit : sigmap(intersection_sig))
				tribuf_signals.insert(bit);
			module->design->scratchpad_set_bool("tribuf.added_something", true);
			log("Merged tribufs driving %s into %s\n", log_signal(intersection_sig), log_id(new_tribuf));
		}
		check_tribuf_signals_consistency(tribuf_signals);
	}
};
